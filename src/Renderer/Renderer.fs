...
// Menu for changing wire display style
let styleMenu dispatch =
    let sheetDispatch sMsg = dispatch (Sheet sMsg)
    let dispatch = Sheet.KeyPress >> sheetDispatch
    
    let devToolsKey = if isMac then "Alt+Command+I" else "Ctrl+Shift+I"
    makeMenu false "Style" [
        makeItem "Classic" None (fun ev -> sheetDispatch (Sheet.Msg.Wire (BusWire.Msg.UpdateStyle BusWire.WireStyle.Classic)))
        makeItem "Radiused" None (fun ev -> sheetDispatch (Sheet.Msg.Wire (BusWire.Msg.UpdateStyle BusWire.WireStyle.Radiused)))
    ]
