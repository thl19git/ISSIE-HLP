...
type KeyboardMsg =
  | CtrlS | CtrlC | CtrlV | CtrlZ | CtrlY | CtrlA | CtrlW | AltC | AltV | AltZ | AltShiftZ | ZoomIn | ZoomOut | DEL | ESC | CtrlR | CtrlF | CtrlP | CtrlM
...
let getWireBBox (wire: BusWire.Wire) (model: Model) =
  let coords = 
      wire.Segments
      |> Array.toList
      |> List.collect (fun seg -> [seg.Start; BusWire.segmentEndCoords seg])
...
let mDragUpdate (model: Model) (mMsg: MouseT) : Model * Cmd<Msg> = 
  ...
  | InitialiseMoving _ ->
      ...
      newModel, Cmd.batch [ cmd; wireCmd (BusWire.ResetJumps) ]
...
let update (msg : Msg) (model : Model): Model*Cmd<Msg> =
  ...
  | KeyPress CtrlR ->
      ...
      Cmd.batch [
          ...
          Cmd.ofMsg UpdateBoundingBoxes
          wireCmd (BusWire.UpdateWires (model.SelectedComponents, posDiff model.LastValidPos model.LastValidPos))
          wireCmd (BusWire.MakeJumps)
  ...
  | KeyPress CtrlF ->
      ...
      Cmd.batch [
          ...
          Cmd.ofMsg UpdateBoundingBoxes
          wireCmd (BusWire.UpdateWires (model.SelectedComponents, posDiff model.LastValidPos model.LastMousePos))
          wireCmd (BusWire.MakeJumps)
      ]
  | KeyPress CtrlP ->
      model,
      symbolCmd (Symbol.ChangeSelectedPort model.SelectedComponents)
  | KeyPress CtrlM ->
      model,
      Cmd.batch [
          symbolCmd (Symbol.MoveSelectedPort model.SelectedComponents)
          wireCmd (BusWire.UpdateWires (model.SelectedComponents, posDiff model.LastValidPos model.LastMousePos))
          wireCmd (BusWire.MakeJumps)
      ]
  ...
  | ManualKeyDown key -> // Needed for e.g. Ctrl + C and Ctrl + V as they are not picked up by Electron
      ...
      let newCmd =
          ...
          | true ->
              ...
              elif Set.contains "R" newPressedKeys then
                  Cmd.ofMsg (KeyPress CtrlR)
              elif Set.contains "F" newPressedKeys then
                  Cmd.ofMsg (KeyPress CtrlF)
              elif Set.contains "P" newPressedKeys then
                  Cmd.ofMsg (KeyPress CtrlP)
              elif Set.contains "M" newPressedKeys then
                  Cmd.ofMsg (KeyPress CtrlM)
              else
                  Cmd.none
          | false -> Cmd.none
